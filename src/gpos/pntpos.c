...
#define EST_VEL     0           /* estimate receiver velocity */
...
static int rescode(int iter, const obsd_t *obs, int n, const double *rs, const double *dts,
                   const double *vare, const int *svh, const nav_t *nav, const double *x, const prcopt_t *opt,
                   double *v, double *H, double *var, double *azel, int *vsat, const int *badflg,
                   double *resp, int *ns)
{
    gtime_t time;
    ...
    for (i=*ns=0;i<n&&i<MAXOBS;i++) {
        ...
        if (i<n-1&&i<MAXOBS-1&&sat==obs[i+1].sat) {
            log_trace(2,"duplicated obs data %s sat=%d\n",time_str(time,3),sat);
            i++;
            continue;
        }
        /* excluded satellite? */
        if (satexclude(sat,vare[i],svh[i],opt)) continue;
        if (badflg[i]) continue;
        ...
        if (iter>0) {
            ...
            if (!(freq=sat2freq(sat,obs[i].code[0],nav))) continue;
            dion*=SQR(FREQ1/freq);
            vion*=SQR(FREQ1/freq);
            
            /* tropospheric correction */
            tropcorr(time,nav,pos,azel+i*2,opt->tropopt,&dtrp,&vtrp);
        }
        /* psendorange with code bias correction */
        ...
        log_trace(4,"sat=%4d azel=%5.1f %4.1f res=%7.3f sig=%5.3f\n",obs[i].sat,
...
static void exclude_bad_sat(const obsd_t *obs, const double *resp, int *badflg, int n)
{
    double vmax;
    int i,j;

    for (j=-1,i=0;i<n;i++) {
        if (resp[i]==0.0) continue;
        if (j<0||vmax<fabs(resp[i])) {vmax=fabs(resp[i]); j=i;}
    }
    if (j>=0) {
        log_trace(1,"exclude satellite: sat=%4d\n",obs[j].sat);
        badflg[j]=1;
    }
}
...
{
    double x[NX]={0},dx[NX],Q[NX*NX],v[MAXOBS+4],H[(MAXOBS+4)*NX],var[MAXOBS],sig;
    double resp_[MAXOBS]={0};
    int i,j,k,e,info,stat,nv,ns,badflg[MAXOBS]={0},max_e=10;
    ...
    for (e=0;e<max_e;e++) {
        ...
        for (i=0;i<MAXITR;i++) {
            ...
            nv=rescode(i,obs,n,rs,dts,vare,svh,nav,x,opt,v,H,var,azel,vsat,badflg,resp,
                    &ns);

            if (i==0) matcpy(resp_,resp,1,n);

            if (nv<NX) {
                sprintf(msg,"lack of valid sats ns=%d",nv);
                if (n>NX) info=-1; break;
            }
            /* weighted by Std */
            for (j=0;j<nv;j++) {
                sig=sqrt(var[j]);
            ...
            if (norm(dx,NX)<1E-4) {
                ...
                if ((stat=valsol(sol,azel,vsat,n,opt,v,nv,NX,msg))) {
                    ...
                    exclude_bad_sat(obs,resp,badflg,n);
                    break;
                }
                return stat;
            }
        ...
        if (i>=MAXITR||info<0) {
            exclude_bad_sat(obs,resp_,badflg,n);
            continue;
        }
    }
    if (i>=max_e) sprintf(msg,"iteration divergent i=%d",i);
    return 0;
}
                    ...
                    double *azel, int *vsat, double *resp, char *msg)
{
    ...
    for (i=0;i<n;i++) {
        ...
        if (!estpos(obs_e,n-1,rs_e,dts_e,vare_e,svh_e,nav,opt,&sol_e,azel_e,
                    vsat_e,resp_e,msg_e)) {
            log_trace(3,"raim_fde: exsat=%2d (%s)\n",obs[i].sat,msg);
            continue;
        }
        ...
        if (nvsat<5) {
            log_trace(3,"raim_fde: exsat=%2d lack of satellites nvsat=%2d\n",
                  obs[i].sat,nvsat);
            continue;
        }
        rms_e=sqrt(rms_e/nvsat);
        
        log_trace(1,"raim_fde: exsat=%2d rms=%8.3f\n",obs[i].sat,rms_e);
    ...
    if (stat) {
        ...
        log_trace(2,"%s: %s excluded by raim\n",tstr+11,name);
    }
    free(obs_e);
...
{
    double x[4]={0},dx[4],Q[16],v[MAXOBS],H[MAXOBS*4];
...
{
    prcopt_t opt_=*opt;
    double rs[MAXOBS*6],dts[MAXOBS*2],var[MAXOBS],azel_[MAXOBS*2],resp[MAXOBS];
    ...
    if (n<=0) {
        strcpy(msg,"no rover observation data");
        return 0;
    }
    sol->time=obs[0].time;
    msg[0]='\0';
    ...
    /* satellite position velocities and clocks */
    ...
    if (ssat) {
        for (i=0;i<MAXSAT;i++) {
            for (j=0;j<NFREQ;j++) ssat[i].snr[j]=0;
            for (j=0;j<NFREQ;j++) ssat[i].resp[j]=ssat[i].resc[j]=0.0;
            ssat[i].vs=0;
        ...
        for (i=0;i<n;i++) {
            ...
            for (j=0;j<NFREQ;j++) ssat[obs[i].sat-1].snr[j]=obs[i].SNR[j];
            ssat[obs[i].sat-1].var=var[i];
            ssat[obs[i].sat-1].svh=svh[i];
            matcpy(ssat[obs[i].sat-1].rs,rs+6*i,1,6);
            matcpy(ssat[obs[i].sat-1].dts,dts+2*i,1,2);
            if (!vsat[i]) continue;
