...
static int fixsol(rtk_t *rtk, const double *v, const double *H, int nv)
{
    ...
    if (!(info=filter(rtk->x,rtk->P,H,v,R,rtk->nx,nv,NULL,0))) {
                       ...
                       const double *azel)
{
    ...
    log_trace(3,"resamb_WLNL: time=%s ns=%d\n",time_str(obs[0].time,0),ns);
    ...
    for (j=0;j<ns;j++) {
        ...
        log_trace(3,"%s sat=%2d-%2d B=%13.3f %13.3f sig=%7.3f %7.3f %s\n",
                       ...
                       const double *azel)
{
    ...
    log_trace(3,"resamb_TCAR: time=%s ns=%d\n",time_str(obs[0].time,0),ns);
    ...
    for (j=0;j<ns;j++) {
        ...
        log_trace(1,"%s sat=%2d-%2d B=%13.3f %13.3f %13.3f sig=%7.3f %7.3f %7.3f %s\n",
