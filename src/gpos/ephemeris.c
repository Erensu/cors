...
extern void alm2pos(gtime_t time, const alm_t *alm, double *rs, double *dts)
{
    ...
    log_trace(4,"alm2pos : time=%s sat=%2d\n",time_str(time,3),alm->sat);
    ...
    if (n>=MAX_ITER_KEPLER) {
        log_trace(2,"alm2pos: kepler iteration overflow sat=%2d\n",alm->sat);
        return;
    }
...
extern double eph2clk(gtime_t time, const eph_t *eph)
{
    ...
    log_trace(4,"eph2clk : time=%s sat=%2d\n",time_str(time,3),eph->sat);
                    ...
                    double *var)
{
    ...
    log_trace(4,"eph2pos : time=%s sat=%2d\n",time_str(time,3),eph->sat);
    ...
    if (n>=MAX_ITER_KEPLER) {
        log_trace(2,"eph2pos: kepler iteration overflow sat=%2d\n",eph->sat);
        return;
    }
    sinE=sin(E); cosE=cos(E);
    
    log_trace(4,"kepler: sat=%2d e=%8.5f n=%2d del=%10.3e\n",eph->sat,eph->e,n,E-Ek);
...
extern double geph2clk(gtime_t time, const geph_t *geph)
{
    ...
    log_trace(4,"geph2clk: time=%s sat=%2d\n",time_str(time,3),geph->sat);
                     ...
                     double *var)
{
    ...
    log_trace(4,"geph2pos: time=%s sat=%2d\n",time_str(time,3),geph->sat);
...
extern double seph2clk(gtime_t time, const seph_t *seph)
{
    ...
    log_trace(4,"seph2clk: time=%s sat=%2d\n",time_str(time,3),seph->sat);
                     ...
                     double *var)
{
    ...
    log_trace(4,"seph2pos: time=%s sat=%2d\n",time_str(time,3),seph->sat);
...
static eph_t *seleph(gtime_t time, int sat, int iode, const nav_t *nav)
{
    ...
    log_trace(4,"seleph  : time=%s sat=%2d iode=%d\n",time_str(time,3),sat,iode);
    ...
    if (iode>=0||j<0) {
        log_trace(3,"no broadcast ephemeris: %s sat=%2d iode=%3d\n",
...
static geph_t *selgeph(gtime_t time, int sat, int iode, const nav_t *nav)
{
    ...
    log_trace(4,"selgeph : time=%s sat=%2d iode=%2d\n",time_str(time,3),sat,iode);
    ...
    if (iode>=0||j<0) {
        log_trace(3,"no glonass ephemeris  : %s sat=%2d iode=%2d\n",time_str(time,0),
              sat,iode);
        return NULL;
    }
    return nav->geph+j;
...
static seph_t *selseph(gtime_t time, int sat, const nav_t *nav)
{
    ...
    log_trace(4,"selseph : time=%s sat=%2d\n",time_str(time,3),sat);
    ...
    if (j<0) {
        log_trace(3,"no sbas ephemeris     : %s sat=%2d\n",time_str(time,0),sat);
        return NULL;
    }
    return nav->seph+j;
}
                  ...
                  double *dts)
{
    ...
    log_trace(4,"ephclk  : time=%s sat=%2d\n",time_str(time,3),sat);
                  ...
                  int iode, double *rs, double *dts, double *var, int *svh)
{
    ...
    log_trace(4,"ephpos  : time=%s sat=%2d iode=%d\n",time_str(time,3),sat,iode);
                        ...
                        double *rs, double *dts, double *var, int *svh)
{
    ...
    log_trace(4,"satpos_sbas: time=%s sat=%2d\n",time_str(time,3),sat);
    ...
    if (i>=nav->sbssat.nsat) {
        log_trace(2,"no sbas correction for orbit: %s sat=%2d\n",time_str(time,0),sat);
                      ...
                      int opt, double *rs, double *dts, double *var, int *svh)
{
    ...
    log_trace(4,"satpos_ssr: time=%s sat=%2d\n",time_str(time,3),sat);
    
    ssr=nav->ssr+sat-1;
    
    if (!ssr->t0[0].time) {
        log_trace(2,"no ssr orbit correction: %s sat=%2d\n",time_str(time,0),sat);
        return 0;
    }
    if (!ssr->t0[1].time) {
        log_trace(2,"no ssr clock correction: %s sat=%2d\n",time_str(time,0),sat);
        return 0;
    }
    /* inconsistency between orbit and clock correction */
    if (ssr->iod[0]!=ssr->iod[1]) {
        log_trace(2,"inconsist ssr correction: %s sat=%2d iod=%d %d\n",
    ...
    if (fabs(t1)>MAXAGESSR||fabs(t2)>MAXAGESSR) {
        log_trace(2,"age of ssr error: %s sat=%2d t=%.0f %.0f\n",time_str(time,0),
              sat,t1,t2);
        *svh=-1;
        return 0;
    }
    ...
    if (norm(deph,3)>MAXECORSSR||fabs(dclk)>MAXCCORSSR) {
        log_trace(3,"invalid ssr correction: %s deph=%.1f dclk=%.1f\n",
    ...
    *var=var_urassr(ssr->ura);
    ...
    log_trace(5,"satpos_ssr: %s sat=%2d deph=%6.3f %6.3f %6.3f er=%6.3f %6.3f %6.3f dclk=%6.3f var=%6.3f\n",
...
extern int satpos(gtime_t time, gtime_t teph, int sat, int ephopt,
                  ...
                  int *svh)
{
    log_trace(4,"satpos  : time=%s sat=%2d ephopt=%d\n",time_str(time,3),sat,ephopt);
                    ...
                    int ephopt, double *rs, double *dts, double *var, int *svh)
{
    ...
    log_trace(3,"satposs : teph=%s n=%d ephopt=%d\n",time_str(teph,3),n,ephopt);
    ...
    for (i=0;i<n&&i<2*MAXOBS;i++) {
        ...
        if (j>=NFREQ) {
            log_trace(3,"no pseudorange %s sat=%2d\n",time_str(obs[i].time,3),obs[i].sat);
            continue;
        }
        ...
        if (!ephclk(time[i],teph,obs[i].sat,nav,&dt)) {
            log_trace(3,"no broadcast clock %s sat=%2d\n",time_str(time[i],3),obs[i].sat);
            continue;
        }
        ...
        if (!satpos(time[i],teph,obs[i].sat,ephopt,nav,rs+i*6,dts+i*2,var+i,
                svh+i)) {
            log_trace(3,"no ephemeris %s sat=%2d\n",time_str(time[i],3),obs[i].sat);
            continue;
        }
...
/* compute distance between satellite and receiver-----------------------------*/
extern double satdis(const double *rr, const obsd_t *obs, const nav_t *nav, int ephopt, double *dts_, double *e)
{
    gtime_t time,tk;
    double dt,pr,rs[6],dts[2],var,r;
    int i,j,svh;

    if (obs->sat<=0||norm(rr,3)<=0.0) return 0.0;

    for (i=0,pr=0.0;i<NFREQ;i++) {
        if ((pr=obs->P[i])!=0.0) break;
    }
    time=timeadd(obs->time,pr?-pr/CLIGHT:-0.075);

    for (r=i=0;i<MAX_ITER;i++) {
        tk=time;
        if (!satpos(tk,tk,obs->sat,ephopt,nav,rs,dts,&var,&svh)) continue;
        if (!(r=geodist(rs,rr,e))) continue;

        time=timeadd(obs->time,-r/CLIGHT);
        if (fabs(timediff(tk,time))<1E-12) break;
    }
    if (dts_) *dts_=dts[0];
    return r;
}
