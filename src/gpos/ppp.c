...
static void detslp_gf(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    for (i=0;i<n&&i<MAXOBS;i++) {
        ...
        log_trace(4,"detslip_gf: sat=%2d gf0=%8.3f gf1=%8.3f\n",obs[i].sat,g0,g1);
...
static void detslp_mw(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    for (i=0;i<n&&i<MAXOBS;i++) {
        ...
        log_trace(4,"detslip_mw: sat=%2d mw0=%8.3f mw1=%8.3f\n",obs[i].sat,w0/0.86,w1/0.86);
...
static void udbias_ppp(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    for (f=0;f<NF(&rtk->opt);f++) {
        ...
        for (i=0;i<n&&i<MAXOBS;i++) {
            ...
            log_trace(5,"udbias_ppp: sat=%2d bias=%.3f\n",sat,bias[i]);
        }
    }
}
                   ...
                   double *azel)
{
    ...
    for (i=0;i<n&&i<MAXOBS;i++) {
        ...
        for (j=0;j<2*NF(opt);j++) {
            ...
            log_trace(4,"%s sat=%2d %s%d res=%9.4f sig=%9.4f el=%4.1f\n",str,sat,
            ...
            if (!post&&opt->maxinno>0.0&&fabs(v[nv])>opt->maxinno) {
                log_trace(2,"outlier (%d) rejected %s sat=%2d %s%d res=%9.4f el=%4.1f\n",
...
extern void pppos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    time2str(obs[0].time,str,2);
    log_trace(3,"pppos   : time=%s nx=%d n=%d\n",str,rtk->nx,n);
    ...
    for (i=0;i<MAX_ITER;i++) {
        ...
        if (!(nv=ppp_res(0,obs,n,rs,dts,var,svh,dr,exc,nav,xp,rtk,v,H,R,azel))) {
            log_trace(2,"%s ppp (%d) no valid obs data\n",str,i+1);
            break;
        }
        /* measurement update of ekf states */
        if ((info=filter(xp,Pp,H,v,R,rtk->nx,nv,NULL,0))) {
            log_trace(2,"%s ppp (%d) filter error info=%d\n",str,i+1,info);
            break;
        }
        /* postfit residuals */
    ...
    if (i>=MAX_ITER) {
        log_trace(2,"%s ppp (%d) iteration overflows\n",str,i);
    }
    if (stat==SOLQ_PPP) {
