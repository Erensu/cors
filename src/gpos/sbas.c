...
static int decode_sbstype1(const sbsmsg_t *msg, sbssat_t *sbssat)
{
    ...
    log_trace(5,"decode_sbstype1: nprn=%d iodp=%d\n",n,sbssat->iodp);
    return 1;
}
...
static int decode_sbstype2(const sbsmsg_t *msg, sbssat_t *sbssat)
{
    ...
    log_trace(5,"decode_sbstype2: type=%d iodf=%d\n",type,iodf);
    return 1;
}
...
static int decode_sbstype6(const sbsmsg_t *msg, sbssat_t *sbssat)
{
    ...
    log_trace(5,"decode_sbstype6: iodf=%d %d %d %d\n",iodf[0],iodf[1],iodf[2],iodf[3]);
    return 1;
}
...
static int decode_sbstype9(const sbsmsg_t *msg, nav_t *nav)
{
    ...
    log_trace(4,"decode_sbstype9:\n");
    
    if (!(sat=satno(SYS_SBS,msg->prn))) {
        log_trace(2,"invalid prn in sbas type 9: prn=%3d\n",msg->prn);
        return 0;
    }
    ...
    log_trace(5,"decode_sbstype9: prn=%d\n",msg->prn);
    return 1;
}
...
static int decode_sbstype18(const sbsmsg_t *msg, sbsion_t *sbsion)
{
    ...
    log_trace(5,"decode_sbstype18: band=%d nigp=%d\n",band,n);
    return 1;
}
...
static int decode_longcorr0(const sbsmsg_t *msg, int p, sbssat_t *sbssat)
{
    ...
    log_trace(5,"decode_longcorr0:sat=%2d\n",sbssat->sat[n-1].sat);
    return 1;
}
...
extern int sbsupdatecorr(const sbsmsg_t *msg, nav_t *nav)
{
    ...
    log_trace(3,"sbsupdatecorr: type=%d\n",type);
    ...
    switch (type) {
        ...
        /*default: log_trace(2,"unsupported sbas message: type=%d\n",type); break;*/
    }
    return stat?type:-1;
}
/* read sbas log file --------------------------------------------------------*/
                     ...
                     sbs_t *sbs)
{
    ...
    log_trace(3,"readmsgs: file=%s sel=%d\n",file,sel);
    
    if (!(fp=fopen(file,"r"))) {
        log_trace(2,"sbas message file open error: %s\n",file);
        return;
    }
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        if (sbs->n>=sbs->nmax) {
            ...
            if (!(sbs_msgs=(sbsmsg_t *)realloc(sbs->msgs,sbs->nmax*sizeof(sbsmsg_t)))) {
                log_trace(1,"readsbsmsg malloc error: nmax=%d\n",sbs->nmax);
                       ...
                       sbs_t *sbs)
{
    ...
    log_trace(3,"sbsreadmsgt: file=%s sel=%d\n",file,sel);
...
extern int sbsreadmsg(const char *file, int sel, sbs_t *sbs)
{
    ...
    log_trace(3,"sbsreadmsg: file=%s sel=%d\n",file,sel);
    
    return sbsreadmsgt(file,sel,ts,te,sbs);
}
/* output sbas messages --------------------------------------------------------
* output sbas message record to output file in gpos sbas log format
                      ...
                      const sbsigp_t **igp, double *x, double *y)
{
    ...
    log_trace(4,"searchigp: pos=%.3f %.3f\n",pos[0]*R2D,pos[1]*R2D);
...
*          in navigation data (data->sbsion) must be set by callig
*          sbsupdatecorr()
*-----------------------------------------------------------------------------*/
                      ...
                      const double *azel, double *delay, double *var)
{
    ...
    log_trace(4,"sbsioncorr: pos=%.3f %.3f azel=%.3f %.3f\n",pos[0]*R2D,pos[1]*R2D,
    ...
    *delay=*var=0.0;
    ...
    if (err) {
        log_trace(2,"no sbas iono correction: lat=%3.0f lon=%4.0f\n",posp[0]*R2D,
              posp[1]*R2D);
        return 0;
    }
    for (i=0;i<4;i++) {
    ...
    *delay*=fp; *var*=fp*fp;
    ...
    log_trace(5,"sbsioncorr: dion=%7.2f sig=%7.2f\n",*delay,sqrt(*var));
    return 1;
}
                          ...
                          double *var)
{
    ...
    log_trace(4,"sbstropcorr: pos=%.3f %.3f azel=%.3f %.3f\n",pos[0]*R2D,pos[1]*R2D,
          azel[0]*R2D,azel[1]*R2D);
                       ...
                       double *drs, double *ddts)
{
    ...
    log_trace(3,"sbslongcorr: sat=%2d\n",sat);
    ...
    for (p=sbssat->sat;p<sbssat->sat+sbssat->nsat;p++) {
        ...
        if (fabs(t)>MAXSBSAGEL) {
            log_trace(2,"sbas long-term correction expired: %s sat=%2d t=%5.0f\n",
        ...
        *ddts=p->lcorr.daf0+p->lcorr.daf1*t;
        ...
        log_trace(5,"sbslongcorr: sat=%2d drs=%7.2f%7.2f%7.2f ddts=%7.2f\n",
    ...
    log_trace(2,"no sbas long-term correction: %s sat=%2d\n",time_str(time,0),sat);
    return 0;
}
/* fast correction -----------------------------------------------------------*/
                       ...
                       double *prc, double *var)
{
    ...
    log_trace(3,"sbsfastcorr: sat=%2d\n",sat);
    ...
    for (p=sbssat->sat;p<sbssat->sat+sbssat->nsat;p++) {
        ...
        *var=varfcorr(p->fcorr.udre)+degfcorr(p->fcorr.ai)*t*t/2.0;
        ...
        log_trace(5,"sbsfastcorr: sat=%3d prc=%7.2f sig=%7.2f t=%5.0f\n",sat,
              *prc,sqrt(*var),t);
        return 1;
    }
    log_trace(2,"no sbas fast correction: %s sat=%2d\n",time_str(time,0),sat);
    return 0;
}
...
*          in navigation data (data->sbssat) must be set by callig
*          sbsupdatecorr().
                      ...
                      double *dts, double *var)
{
    ...
    log_trace(5,"sbssatcorr: sat=%2d drs=%6.3f %6.3f %6.3f dclk=%.3f %.3f var=%.3f\n",
                        ...
                        sbsmsg_t *sbsmsg)
{
    ...
    log_trace(5,"sbsdecodemsg: prn=%d\n",prn);
