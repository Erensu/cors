...
#define C_CTRQ      (char)0x11          /*  quit command (ctrl-q) */
...
static dev_win_t dev_win;

static HANDLE open_win(const char *path, int opt)
{
    HANDLE handle;
    switch (opt) {
        case O_RDWR:
            handle=CreateFile(path,GENERIC_READ|GENERIC_WRITE,0,NULL,CREATE_ALWAYS,0,NULL);
            break;
        case O_RDONLY:
            handle=CreateFile(path,GENERIC_READ,0,NULL,CREATE_ALWAYS,0,NULL);
            break;
        case O_WRONLY:
            handle=CreateFile(path,GENERIC_WRITE,0,NULL,CREATE_ALWAYS,0,NULL);
            break;
        default:
            break;
    }
...
static int close_win(int fd)
{
    if (fd>0) {
        closesocket(fd);
    }
    else if (fd==0) {
        CloseHandle(dev_win.hout);
        CloseHandle(dev_win.hin);
    }
}

static ssize_t write_win(int fd, const void* buffer, size_t count)
{
    int rc=0,ret;

    ret=WriteFile(dev_win.hout,buffer,count,&rc,NULL);
    if (ret==0) return -1;
    else return rc;
}

static ssize_t read_win(int fd, void* buffer, size_t count)
{
    int rc=0,ret;

    ret=ReadFile(dev_win.hin,buffer,count,&rc,NULL);
    if (ret==0) return -1;
    else return rc;
}
...
#define open  open_win
#define close close_win
#endif
...
*-----------------------------------------------------------------------------*/
extern vt_t *vt_open(const char *dev)
{
    vt_t *vt;
    int i;
    
    log_trace(3,"vt_open: dev=%s\n",dev);
...
#ifdef WIN32
    dev_win.hout=open("CONOUT$",O_WRONLY);
    dev_win.hin=open("CONIN$",O_RDONLY);

    if (dev_win.hin==INVALID_HANDLE_VALUE||dev_win.hout==INVALID_HANDLE_VALUE) {
        free(vt);
        return 0;
    }
#else
...
#ifdef WIN32
    if (vt->type==0) {
        if (read(vt->in,c,1)!=1) return 0; /* error */
    }
    else if (vt->type==1) {
