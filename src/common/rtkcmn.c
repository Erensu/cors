/*------------------------------------------------------------------------------
* rtkcmn.c : gpos common functions
*
* options : -DLAPACK   use LAPACK/BLAS
*           -DMKL      use Intel MKL
*           -Dlog_trace    enable debug log_trace
...
const prcopt_t prcopt_default={ /* defaults processing options */
        PMODE_SINGLE,0,2,SYS_GPS|SYS_CMP|SYS_QZS|SYS_GAL|SYS_GLO,   /* mode,soltype,nf,navsys */
        15.0*D2R,{{0,0}},           /* elmin,snrmask */
        0,1,1,1,                    /* sateph,modear,glomodear,bdsmodear */
        5,0,3,1,                    /* maxout,minlock,minfix,armaxiter */
...
const prcopt_t prcopt_default_pnt={ /* defaults processing options */
        PMODE_SINGLE,0,3,SYS_GPS|SYS_CMP|SYS_QZS|SYS_GAL|SYS_GLO,   /* mode,soltype,nf,navsys */
        15.0*D2R,{{0,0}},           /* elmin,snrmask */
        0,1,1,1,                    /* sateph,modear,glomodear,bdsmodear */
        5,0,3,1,                    /* maxout,minlock,minfix,armaxiter */
        0,0,0,0,                    /* estion,esttrop,dynamics,tidecorr */
...
const prcopt_t prcopt_default_rtk={ /* defaults processing options */
        PMODE_KINEMA,0,3,SYS_GPS|SYS_CMP|SYS_QZS|SYS_GAL|SYS_GLO,   /* mode,soltype,nf,navsys */
        15.0*D2R,{{0,0},{{30.0,30.0,30.0,30.0,30.0,30.0,30.0,30.0,30.0},
...
const solopt_t solopt_default={ /* defaults solution output options */
        ...
        0,0,0,                      /* solstatic,sstat,log_trace */
        {0.0,0.0},                  /* nmeaintv */
...
extern int satexclude(int sat, double var, int svh, const prcopt_t *opt)
{
    ...
    if (svh) {
        log_trace(3,"unhealthy satellite: sat=%3d svh=%02X\n",sat,svh);
        return 1;
    }
    if (var>MAX_VAR_EPH) {
        log_trace(3,"invalid ura satellite: sat=%3d ura=%.2f\n",sat,sqrt(var));
        return 1;
    }
    return 0;
}
...
{
    log_trace(3,"setcodepri:sys=%d idx=%d pri=%s\n",sys,idx,pri);
...
extern int decode_word(uint32_t word, uint8_t *data)
{
    ...
    log_trace(5,"decodeword: word=%08x\n",word);
...
extern int *imat(int n, int m)
{
    ...
    if (n<=0||m<=0) return NULL;
    if (!(p=(int *)calloc(n*m,sizeof(int)))) {
...
{
    int i,j;
    for (i=0;i<n;i++) for (j=0;j<m;j++) At[i+j*n]=A[j+i*m];
}
...
*-----------------------------------------------------------------------------*/
extern inline uint32_t tickget(void)
{
#ifdef WIN32
...
static void addpcv(const pcv_t *pcv, pcvs_t *pcvs)
{
    ...
    if (pcvs->nmax<=pcvs->n) {
        ...
        if (!(pcvs_pcv=(pcv_t *)realloc(pcvs->pcv,sizeof(pcv_t)*pcvs->nmax))) {
            log_trace(1,"addpcv: memory allocation error\n");
...
static int readngspcv(const char *file, pcvs_t *pcvs)
{
    ...
    if (!(fp=fopen(file,"r"))) {
        log_trace(2,"ngs pcv file open error: %s\n",file);
        return 0;
    }
...
static int readantex(const char *file, pcvs_t *pcvs)
{
    ...
    log_trace(3,"readantex: file=%s\n",file);

    if (!(fp=fopen(file,"r"))) {
        log_trace(2,"antex pcv file open error: %s\n",file);
        return 0;
    }
    while (fgets(buff,sizeof(buff),fp)) {
...
extern int readpcv(const char *file, pcvs_t *pcvs)
{
    ...
    for (i=0;i<pcvs->n;i++) {
        pcv=pcvs->pcv+i;
        log_trace(4,"sat=%2d type=%20s code=%s off=%8.4f %8.4f %8.4f  %8.4f %8.4f %8.4f\n",
                        ...
                        const pcvs_t *pcvs)
{
    ...
    log_trace(3,"searchpcv: sat=%2d type=%s\n",sat,type);
    ...
    if (sat) { /* search satellite antenna */
        ...
        for (i=0;i<pcvs->n;i++) {
            ...
            log_trace(2,"pcv without radome is used type=%s\n",type);
            return pcv;
        }
    }
    return NULL;
...
extern int readblq(const char *file, const char *sta, double *odisp)
{
    ...
    if (!(fp=fopen(file,"r"))) {
        log_trace(2,"blq file open error: file=%s\n",file);
        return 0;
    }
    while (fgets(buff,sizeof(buff),fp)) {
    ...
    fclose(fp);
    log_trace(2,"no otl parameters: sta=%s file=%s\n",sta,file);
    return 0;
}
...
extern int readerp(const char *file, erp_t *erp)
{
    ...
    log_trace(3,"readerp: file=%s\n",file);

    if (!(fp=fopen(file,"r"))) {
        log_trace(2,"erp file open error: file=%s\n",file);
        return 0;
    }
    while (fgets(buff,sizeof(buff),fp)) {
...
static void uniqeph(nav_t *nav)
{
    ...
    if (!(nav_eph=(eph_t *)realloc(nav->eph,sizeof(eph_t)*nav->n))) {
        log_trace(1,"uniqeph malloc error n=%d\n",nav->n);
...
static void uniqgeph(nav_t *nav)
{
    ...
    if (!(nav_geph=(geph_t *)realloc(nav->geph,sizeof(geph_t)*nav->ng))) {
        log_trace(1,"uniqgeph malloc error ng=%d\n",nav->ng);
...
static void uniqseph(nav_t *nav)
{
    ...
    if (!(nav_seph=(seph_t *)realloc(nav->seph,sizeof(seph_t)*nav->ns))) {
        log_trace(1,"uniqseph malloc error ns=%d\n",nav->ns);
    ...
    log_trace(4,"uniqseph: ns=%d\n",nav->ns);
}
/* unique ephemerides ----------------------------------------------------------
* unique ephemerides in navigation data
* args   : nav_t *data    IO     navigation data
* return : number of epochs
*-----------------------------------------------------------------------------*/
extern void uniqnav(nav_t *nav)
{
    log_trace(3,"uniqnav: neph=%d ngeph=%d nseph=%d\n",nav->n,nav->ng,nav->ns);
...
extern int expath(const char *path, char *paths[], int nmax)
{
    ...
    log_trace(3,"expath  : path=%s nmax=%d\n",path,nmax);
    ...
    for (i=0;i<n;i++) log_trace(3,"expath  : file=%s\n",paths[i]);
...
#endif
    log_trace(2,"directory generation error: dir=%s\n",dir);
    return 0;
}
/* replace string ------------------------------------------------------------*/
...
#endif
    log_trace(4,"tropmapf: pos=%10.6f %11.6f %6.1f azel=%5.1f %4.1f\n",
                     ...
                     int opt, double *dant)
{
    ...
    log_trace(4,"antmodel: azel=%6.1f %4.1f opt=%d\n",azel[0]*R2D,azel[1]*R2D,opt);
    ...
    log_trace(5,"antmodel: dant=%6.3f %6.3f\n",dant[0],dant[1]);
}
...
extern void antmodel_s(const pcv_t *pcv, double nadir, double *dant)
{
    ...
    log_trace(4,"antmodel_s: nadir=%6.1f\n",nadir*R2D);

    for (i=0;i<NFREQ;i++) {
        dant[i]=interpvar(nadir*R2D*5.0,pcv->var[i]);
    }
    log_trace(5,"antmodel_s: dant=%6.3f %6.3f\n",dant[0],dant[1]);
}
...
static void sunmoonpos_eci(gtime_t tut, double *rsun, double *rmoon)
{
    ...
    log_trace(4,"sunmoonpos_eci: tut=%s\n",time_str(tut,3));
...
extern char *strrstr(const char *dst, const char *src)
{
    const char *pdst=dst;
    const char *psrc=src;
    char *right=NULL;
    ...
    while (*dst) {
        while (*pdst==*psrc) {
            if (*pdst=='\0') return right=(char *)dst;
            else {pdst++; psrc++;}
        }
        if (*psrc=='\0') right=(char *)dst;
        pdst=++dst;
        psrc=src;
    }
    return right;
...
extern int pnpoly(int nvert, const double *vertx, const double *verty, double px, double py)
{
    int i,j,c=0;
...
/* median of input vector----------------------------------------------------*/
extern double median(const double *vec,int n)
{
    double *v,m;

    if (n<=2) return avg(vec,n);
    v=mat(1,n);

    matcpy(v,vec,1,n);
    qsort(v,n,sizeof(double),cmpres);

    if (n%2==0) m=(v[n/2-1]+v[n/2])/2.0;
    else m=v[n/2];
    free(v); return m;
}
/* mad of input vector--------------------------------------------------------*/
extern double mad(const double *vec,int n)
{
    ...
    for (i=0;i<n;i++) vm[i]=fabs(vec[i]-m);
    m=median(vm,n);

    free(v); free(vm); return m;
}
...
extern int decbase64(const char *indata, int inlen, char *outdata, int *outlen)
{
