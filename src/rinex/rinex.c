...
static void convcode(double ver, int sys, const char *str, char *type)
{
    ...
    log_trace(3,"convcode: ver=%.2f sys=%2d type= %s -> %s\n",ver,sys,str,type);
}
                        ...
                        char tobs[][MAXOBSTYPE][4], nav_t *nav, sta_t *sta)
{
    ...
    log_trace(4,"decode_obsh: ver=%.2f\n",ver);
    ...
    if      (strstr(label,"MARKER NAME"         )) {
        ...
        if (!(p=strchr(syscodes,buff[0]))) {
            log_trace(2,"invalid system code: sys=%c\n",buff[0]);
            return;
        }
        i=(int)(p-syscodes);
        ...
        *tobs[i][nt]='\0';
        ...
        for (j=0;j<nt;j++) {
            ...
            log_trace(2,"set default for unknown code: sys=%c code=%s\n",buff[0],
                  tobs[i][j]);
        }
    }
        ...
        if (strlen(buff)<=60) {
            ...
            *type=*(buff+20);
            ...
            switch (*(buff+40)) {
                ...
                default :
                    log_trace(2,"not supported satellite system: %c\n",*(buff+40));
                    break;
            }
            continue;
        }
                           ...
                           int *flag, int *sats)
{
    ...
    log_trace(4,"decode_obsepoch: ver=%.2f\n",ver);
    ...
    if (ver<=2.99) { /* ver.2 */
        ...
        *flag=(int)str2num(buff,28,1);
        ...
        if (str2time(buff,0,26,time)) {
            log_trace(2,"rinex obs invalid epoch: epoch=%26.26s\n",buff);
            return 0;
        }
        ...
        *flag=(int)str2num(buff,31,1);
        ...
        if (buff[0]!='>'||str2time(buff,1,28,time)) {
            log_trace(2,"rinex obs invalid epoch: epoch=%29.29s\n",buff);
            return 0;
        }
    }
    log_trace(4,"decode_obsepoch: time=%s flag=%d\n",time_str(*time,3),*flag);
    return n;
}
                          ...
                          sigind_t *index, obsd_t *obs)
{
    ...
    log_trace(4,"decode_obsdata: ver=%.2f\n",ver);
    ...
    if (!obs->sat) {
        log_trace(4,"decode_obsdata: unsupported sat sat=%s\n",satid);
        stat=0;
    }
    ...
    log_trace(4,"decode_obsdata: time=%s sat=%2d\n",time_str(obs->time,0),obs->sat);
    return 1;
}
/* save cycle slips ----------------------------------------------------------*/
...
static int addobsdata(obs_t *obs, const obsd_t *data)
{
    ...
    if (obs->nmax<=obs->n) {
        ...
        if (!(obs_data=(obsd_t *)realloc(obs->data,sizeof(obsd_t)*obs->nmax))) {
            log_trace(1,"addobsdata: malloc error n=%dx%d\n",sizeof(obsd_t),obs->nmax);
                      ...
                      char tobs[MAXOBSTYPE][4], sigind_t *ind)
{
    ...
    for (p=opt;p&&(p=strchr(p,'-'));p++) {
        ...
        for (i=0;i<n;i++) {
            ...
            ind->shift[i]=shift;
            log_trace(2,"phase shift: sys=%2d tobs=%s shift=%.3f\n",sys,
                  tobs[i],shift);
        }
    }
    ...
    for (i=0;i<n;i++) {
        ...
        log_trace(4,"reject obs type: sys=%2d, obs=%s\n",sys,tobs[i]);
    }
    ind->n=n;
    
#if 0 /* for debug */
    for (i=0;i<n;i++) {
        log_trace(2,"set_index: sys=%2d,tobs=%s code=%2d pri=%2d idx=%d pos=%d shift=%5.2f\n",
                      ...
                      char tobs[][MAXOBSTYPE][4], obs_t *obs, sta_t *sta)
{
    ...
    log_trace(4,"readrnxobs: rcv=%d ver=%.2f tsys=%d\n",rcv,ver,tsys);
    ...
    log_trace(4,"readrnxobs: nobs=%d stat=%d\n",obs->n,stat);
                      ...
                      eph_t *eph)
{
    ...
    log_trace(4,"decode_eph: ver=%.2f sat=%2d\n",ver,sat);
    
    sys=satsys(sat,NULL);
    
    if (!(sys&(SYS_GPS|SYS_GAL|SYS_QZS|SYS_CMP|SYS_IRN))) {
        log_trace(3,"ephemeris error: invalid satellite sat=%2d\n",sat);
        return 0;
    }
    *eph=eph0;
    ...
    if (eph->iode<0||1023<eph->iode) {
        log_trace(2,"rinex nav invalid: sat=%2d iode=%d\n",sat,eph->iode);
    }
    if (eph->iodc<0||1023<eph->iodc) {
        log_trace(2,"rinex nav invalid: sat=%2d iodc=%d\n",sat,eph->iodc);
    }
    return 1;
}
                       ...
                       geph_t *geph)
{
    ...
    log_trace(4,"decode_geph: ver=%.2f sat=%2d\n",ver,sat);
    
    if (satsys(sat,NULL)!=SYS_GLO) {
        log_trace(3,"glonass ephemeris error: invalid satellite sat=%2d\n",sat);
        return 0;
    }
    *geph=geph0;
    ...
    if (geph->frq<MINFREQ_GLO||MAXFREQ_GLO<geph->frq) {
        log_trace(2,"rinex gnav invalid freq: sat=%2d fn=%d\n",sat,geph->frq);
    }
    return 1;
}
                       ...
                       seph_t *seph)
{
    ...
    log_trace(4,"decode_seph: ver=%.2f sat=%2d\n",ver,sat);
    
    if (satsys(sat,NULL)!=SYS_SBS) {
        log_trace(3,"geo ephemeris error: invalid satellite sat=%2d\n",sat);
        return 0;
    }
    *seph=seph0;
                       ...
                       int *type, eph_t *eph, geph_t *geph, seph_t *seph)
{
    ...
    log_trace(4,"readrnxnavb: ver=%.2f sys=%d\n",ver,sys);
    ...
    while (fgets(buff,MAXRNXLEN,fp)) {
        ...
        if (i==0) {
            ...
            if (str2time(buff+sp,0,19,&toc)) {
                log_trace(2,"rinex nav toc error: %23.23s\n",buff);
                return 0;
            }
            /* decode data fields */
...
static int add_eph(nav_t *nav, const eph_t *eph)
{
    ...
    if (nav->nmax<=nav->n) {
        ...
        if (!(nav_eph=(eph_t *)realloc(nav->eph,sizeof(eph_t)*nav->nmax))) {
            log_trace(1,"decode_eph malloc error: n=%d\n",nav->nmax);
...
static int add_geph(nav_t *nav, const geph_t *geph)
{
    ...
    if (nav->ngmax<=nav->ng) {
        ...
        if (!(nav_geph=(geph_t *)realloc(nav->geph,sizeof(geph_t)*nav->ngmax))) {
            log_trace(1,"decode_geph malloc error: n=%d\n",nav->ngmax);
...
static int add_seph(nav_t *nav, const seph_t *seph)
{
    ...
    if (nav->nsmax<=nav->ns) {
        ...
        if (!(nav_seph=(seph_t *)realloc(nav->seph,sizeof(seph_t)*nav->nsmax))) {
            log_trace(1,"decode_seph malloc error: n=%d\n",nav->nsmax);
                      ...
                      nav_t *nav)
{
    ...
    log_trace(3,"readrnxnav: ver=%.2f sys=%d\n",ver,sys);
...
static int readrnxclk(FILE *fp, const char *opt, int index, nav_t *nav)
{
    ...
    log_trace(3,"readrnxclk: index=%d\n", index);
    ...
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        if (str2time(buff,8,26,&time)) {
            log_trace(2,"rinex clk invalid epoch: %34.34s\n",buff);
            continue;
        }
        ...
        if (nav->nc>=nav->ncmax) {
            ...
            if (!(nav_pclk=(pclk_t *)realloc(nav->pclk,sizeof(pclk_t)*(nav->ncmax)))) {
                log_trace(1,"readrnxclk malloc error: nmax=%d\n",nav->ncmax);
                     ...
                     obs_t *obs, nav_t *nav, sta_t *sta)
{
    ...
    log_trace(3,"readrnxfp: flag=%d index=%d\n",flag,index);
    ...
    log_trace(2,"unsupported rinex type ver=%.2f type=%c\n",ver,*type);
    return 0;
}
                       ...
                       obs_t *obs, nav_t *nav, sta_t *sta)
{
    ...
    log_trace(3,"readrnxfile: file=%s flag=%d index=%d\n",file,flag,index);
    
    if (sta) init_sta(sta);
    
    /* uncompress file */
    if ((cstat=rtk_uncompress(file,tmpfile))<0) {
        log_trace(2,"rinex file uncompact error: %s\n",file);
        return 0;
    }
    if (!(fp=fopen(cstat?tmpfile:file,"r"))) {
        log_trace(2,"rinex file open error: %s\n",cstat?tmpfile:file);
        return 0;
    }
    /* read RINEX file */
                    ...
                    sta_t *sta)
{
    ...
    log_trace(3,"readrnxt: file=%s rcv=%d\n",file,rcv);
                   ...
                   nav_t *nav, sta_t *sta)
{
    ...
    log_trace(3,"readrnx : file=%s rcv=%d\n",file,rcv);
...
static void combpclk(nav_t *nav)
{
    ...
    if (!(nav_pclk=(pclk_t *)realloc(nav->pclk,sizeof(pclk_t)*nav->nc))) {
        free(nav->pclk); nav->pclk=NULL; nav->nc=nav->ncmax=0;
        log_trace(1,"combpclk malloc error nc=%d\n",nav->nc);
        return;
    }
    nav->pclk=nav_pclk;
    nav->ncmax=nav->nc;
    
    log_trace(4,"combpclk: nc=%d\n",nav->nc);
}
...
extern int open_rnxctr(rnxctr_t *rnx, FILE *fp)
{
    ...
    log_trace(3,"open_rnxctr:\n");
    
    /* read RINEX header from file */
    if (!readrnxh(fp,&ver,&type,&sys,&tsys,tobs,&rnx->nav,&rnx->sta)) {
        log_trace(2,"open_rnxctr: rinex header read error\n");
        return 0;
    }
    if (!strchr(rnxtypes,type)) {
        log_trace(2,"open_rnxctr: not supported rinex type type=%c\n",type);
        return 0;
    }
    rnx->ver=ver;
    rnx->type=type;
...
extern int outrnxgnavb(FILE *fp, const rnxopt_t *opt, const geph_t *geph)
{
    ...
    log_trace(3,"outrnxgnavb: sat=%2d\n",geph->sat);
...
extern int outrnxhnavb(FILE *fp, const rnxopt_t *opt, const seph_t *seph)
{
    ...
    log_trace(3,"outrnxhnavb: sat=%2d\n",seph->sat);
...
static void convcode_rnx(int rnxver, int sys, char *type)
{
    ...
    for (i=0;codes[i];i++) {
        ...
        for (j=0;j<4;j++) {
            ...
            if (opt->rnxver<=299) { /* ver.2 */
                ...
                /* ver.3 -> ver.2 */
                convcode_rnx(opt->rnxver,navsys[sys],type);
...
extern void init_rnxopt(rnxopt_t *rnxopt, const obsd_t *obs, int nobs)
{
    rnxopt->rnxver=304;
    ...
    for (i=0;i<6;i++) for (j=0;j<64;j++) rnxopt->mask[i][j]='1';
    uint8_t codes[8][33]={{0}};
    uint8_t types[8][33]={{0}};

    for (i=0;i<nobs;i++) {
        int sys=satsys(obs[i].sat,NULL);
        if (!(sys&rnxopt->navsys)) continue;
    ...
    for (i=0;i<8;i++) {
        ...
        setopt_obstype(codes[i],types[i],i,rnxopt);
    }
}
