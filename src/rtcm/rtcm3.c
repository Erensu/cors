...
static int test_staid(rtcm_t *rtcm, int staid)
{
    ...
    if (rtcm->staid==0||rtcm->obsflag) {
        ...
        log_trace(2,"rtcm3 %d staid invalid id=%d %d\n",type,staid,rtcm->staid);
...
static int decode_head1001(rtcm_t *rtcm, int *sync)
{
    ...
    if (i+52<=rtcm->len*8) {
        ...
        *sync=getbitu(rtcm->buff,i, 1);       i+= 1;
        ...
        log_trace(2,"rtcm3 %d length error: len=%d\n",type,rtcm->len);
        return -1;
    }
    /* test station ID */
    ...
    log_trace(3,"decode_head1001: time=%s nsat=%d sync=%d\n",tstr,nsat,*sync);
...
static int decode_type1002(rtcm_t *rtcm)
{
    ...
    for (j=0;j<nsat&&rtcm->obs.n<MAXOBS&&i+74<=rtcm->len*8;j++) {
        ...
        if (!(sat=satno(sys,prn))) {
            log_trace(2,"rtcm3 1002 satellite number error: prn=%d\n",prn);
            continue;
        }
...
static int decode_type1004(rtcm_t *rtcm)
{
    ...
    for (j=0;j<nsat&&rtcm->obs.n<MAXOBS&&i+125<=rtcm->len*8;j++) {
        ...
        if (!(sat=satno(sys,prn))) {
            log_trace(2,"rtcm3 1004 satellite number error: sys=%d prn=%d\n",sys,prn);
            continue;
        }
        tt=timediff(rtcm->obs.data[0].time,rtcm->time);
        if (rtcm->obsflag||fabs(tt)>1E-9) {
...
static int decode_type1005(rtcm_t *rtcm)
{
    ...
    if (i+140==rtcm->len*8) {
        ...
        log_trace(2,"rtcm3 1005 length error: len=%d\n",rtcm->len);
        return -1;
    }
    if (rtcm->outmsg) {
        msg=rtcm->msgtype+strlen(rtcm->msgtype);
...
static int decode_type1006(rtcm_t *rtcm)
{
    ...
    if (i+156<=rtcm->len*8) {
        ...
        log_trace(2,"rtcm3 1006 length error: len=%d\n",rtcm->len);
        return -1;
    }
    if (rtcm->outmsg) {
        msg=rtcm->msgtype+strlen(rtcm->msgtype);
...
static int decode_type1007(rtcm_t *rtcm)
{
    ...
    if (i+28+8*n<=rtcm->len*8) {
        ...
        log_trace(2,"rtcm3 1007 length error: len=%d\n",rtcm->len);
        return -1;
    }
    if (rtcm->outmsg) {
        msg=rtcm->msgtype+strlen(rtcm->msgtype);
...
static int decode_type1008(rtcm_t *rtcm)
{
    ...
    if (i+36+8*(n+m)<=rtcm->len*8) {
        ...
        log_trace(2,"rtcm3 1008 length error: len=%d\n",rtcm->len);
        return -1;
    }
    if (rtcm->outmsg) {
        msg=rtcm->msgtype+strlen(rtcm->msgtype);
...
static int decode_head1009(rtcm_t *rtcm, int *sync)
{
    ...
    time2str(rtcm->time,tstr,2);
    log_trace(4,"decode_head1009: time=%s nsat=%d sync=%d\n",tstr,nsat,*sync);
...
static int decode_type1010(rtcm_t *rtcm)
{
    ...
    for (j=0;j<nsat&&rtcm->obs.n<MAXOBS&&i+79<=rtcm->len*8;j++) {
        ...
        cnr1 =getbitu(rtcm->buff,i, 8); i+= 8;
        if (!(sat=satno(sys,prn))) {
            log_trace(2,"rtcm3 1010 satellite number error: prn=%d\n",prn);
            continue;
        }
...
static int decode_type1012(rtcm_t *rtcm)
{
    ...
    for (j=0;j<nsat&&rtcm->obs.n<MAXOBS&&i+130<=rtcm->len*8;j++) {
        ...
        cnr2 =getbitu(rtcm->buff,i, 8); i+= 8;
        if (!(sat=satno(sys,prn))) {
            log_trace(2,"rtcm3 1012 satellite number error: sys=%d prn=%d\n",sys,prn);
            continue;
        }
        if (!rtcm->nav.glo_fcn[prn-1]) {
            rtcm->nav.glo_fcn[prn-1]=fcn-7+8; /* fcn+8 */
...
static int decode_type1019(rtcm_t *rtcm)
{
    ...
    if (i+476<=rtcm->len*8) {
        ...
        log_trace(2,"rtcm3 1019 length error: len=%d\n",rtcm->len);
        return -1;
    }
    if (prn>=40) {
        sys=SYS_SBS; prn+=80;
    }
    log_trace(4,"decode_type1019: prn=%d iode=%d toe=%.0f\n",prn,eph.iode,eph.toes);
    ...
    if (!(sat=satno(sys,prn))) {
        log_trace(2,"rtcm3 1019 satellite number error: prn=%d\n",prn);
        return -1;
    }
    eph.sat=sat;
...
static int decode_type1020(rtcm_t *rtcm)
{
    ...
    if (i+348<=rtcm->len*8) {
        ...
        log_trace(2,"rtcm3 1020 length error: len=%d\n",rtcm->len);
        return -1;
    }
    if (!(sat=satno(sys,prn))) {
        log_trace(2,"rtcm3 1020 satellite number error: prn=%d\n",prn);
        return -1;
    }
    log_trace(4,"decode_type1020: prn=%d tk=%02.0f:%02.0f:%02.0f\n",prn,tk_h,tk_m,tk_s);
    ...
    rtcm->nav.glo_fcn[prn-1]=geph.frq+8;
    rtcm->ephsat=sat;
    rtcm->ephset=0;
    return 2;
}
...
static int decode_type1029(rtcm_t *rtcm)
{
    ...
    if (i+60<=rtcm->len*8) {
        ...
        log_trace(2,"rtcm3 1029 length error: len=%d\n",rtcm->len);
        return -1;
    }
    if (i+nchar*8>rtcm->len*8) {
        log_trace(2,"rtcm3 1029 length error: len=%d nchar=%d\n",rtcm->len,nchar);
        return -1;
    }
...
static int decode_type1033(rtcm_t *rtcm)
{
    ...
    if (i+60+8*(n+m+n1+n2+n3)<=rtcm->len*8) {
        ...
        log_trace(2,"rtcm3 1033 length error: len=%d\n",rtcm->len);
        return -1;
    }
    if (rtcm->outmsg) {
        msg=rtcm->msgtype+strlen(rtcm->msgtype);
...
static int decode_type1041(rtcm_t *rtcm)
{
    ...
    if (i+482-12<=rtcm->len*8) {
        ...
        log_trace(2,"rtcm3 1041 length error: len=%d\n",rtcm->len);
        return -1;
    }
    log_trace(4,"decode_type1041: prn=%d iode=%d toe=%.0f\n",prn,eph.iode,eph.toes);
    ...
    if (!(sat=satno(sys,prn))) {
        log_trace(2,"rtcm3 1041 satellite number error: prn=%d\n",prn);
        return -1;
    }
    eph.sat=sat;
    eph.week=adjgpsweek(week);
...
static int decode_type1044(rtcm_t *rtcm)
{
    ...
    if (i+473<=rtcm->len*8) {
        ...
        log_trace(2,"rtcm3 1044 length error: len=%d\n",rtcm->len);
        return -1;
    }
    log_trace(4,"decode_type1044: prn=%d iode=%d toe=%.0f\n",prn,eph.iode,eph.toes);
    ...
    if (!(sat=satno(sys,prn))) {
        log_trace(2,"rtcm3 1044 satellite number error: prn=%d\n",prn);
        return -1;
    }
    eph.sat=sat;
    eph.week=adjgpsweek(week);
...
static int decode_type1045(rtcm_t *rtcm)
{
    ...
    if (i+484<=rtcm->len*8) {
        ...
        log_trace(2,"rtcm3 1045 length error: len=%d\n",rtcm->len);
        return -1;
    }
    log_trace(4,"decode_type1045: prn=%d iode=%d toe=%.0f\n",prn,eph.iode,eph.toes);
    ...
    if (!(sat=satno(sys,prn))) {
        log_trace(2,"rtcm3 1045 satellite number error: prn=%d\n",prn);
        return -1;
    }
...
static int decode_type1046(rtcm_t *rtcm)
{
    ...
    if (i+492<=rtcm->len*8) {
        ...
        log_trace(2,"rtcm3 1046 length error: len=%d\n",rtcm->len);
        return -1;
    }
    log_trace(4,"decode_type1046: prn=%d iode=%d toe=%.0f\n",prn,eph.iode,eph.toes);
    ...
    if (!(sat=satno(sys,prn))) {
        log_trace(2,"rtcm3 1046 satellite number error: prn=%d\n",prn);
        return -1;
    }
...
static int decode_type1042(rtcm_t *rtcm)
{
    ...
    if (i+499<=rtcm->len*8) {
        ...
        log_trace(2,"rtcm3 1042 length error: len=%d\n",rtcm->len);
        return -1;
    }
    log_trace(4,"decode_type1042: prn=%d iode=%d toe=%.0f\n",prn,eph.iode,eph.toes);
    ...
    if (!(sat=satno(sys,prn))) {
        log_trace(2,"rtcm3 1042 satellite number error: prn=%d\n",prn);
        return -1;
    }
    eph.sat=sat;
                            ...
                            int *iod, double *udint, int *refd, int *hsize)
{
    ...
    *udint=ssrudint[udi];
    ...
    time2str(rtcm->time,tstr,2);
    log_trace(4,"decode_ssr1_head: time=%s sys=%d subtype=%d nsat=%d sync=%d iod=%d"
                            ...
                            int *iod, double *udint, int *hsize)
{
    ...
    *udint=ssrudint[udi];
    ...
    time2str(rtcm->time,tstr,2);
    log_trace(4,"decode_ssr2_head: time=%s sys=%d subtype=%d nsat=%d sync=%d iod=%d"
            " provid=%d solid=%d\n",tstr,sys,subtype,nsat,*sync,*iod,provid,solid);
...
static int decode_ssr1(rtcm_t *rtcm, int sys, int subtype)
{
    ...
    for (j=0;j<nsat&&i+121+np+ni+nj<=rtcm->len*8;j++) {
        ...
        if (!(sat=satno(sys,prn))) {
            log_trace(2,"rtcm3 %d satellite number error: prn=%d\n",type,prn);
            continue;
        }
    ...
    for (j=0;j<nsat&&i+5+np<=rtcm->len*8;j++) {
        ...
        for (k=0;k<nbias&&i+19<=rtcm->len*8;k++) {
            ...
            if (sigs[mode]) {
                ...
                log_trace(2,"rtcm3 %d not supported mode: mode=%d\n",type,mode);
            }
        }
        if (!(sat=satno(sys,prn))) {
            log_trace(2,"rtcm3 %d satellite number error: prn=%d\n",type,prn);
                            ...
                            int *hsize)
{
    ...
    *udint=ssrudint[udi];
    ...
    time2str(rtcm->time,tstr,2);
    log_trace(4,"decode_ssr7_head: time=%s sys=%d subtype=%d nsat=%d sync=%d iod=%d"
            " provid=%d solid=%d\n",tstr,sys,subtype,nsat,*sync,*iod,provid,solid);
                     ...
                     int *idx)
{
    ...
    for (i=nex=0;i<n;i++) {
        if (ex[i]==0) ;
            ...
            log_trace(2,"rtcm msm: no space in obs data sys=%d code=%d\n",sys,code[i]);
            idx[i]=-1;
        }
#if 0 /* for debug */
        log_trace(2,"sig pos: sys=%d code=%d ex=%d idx=%d\n",sys,code[i],ex[i],idx[i]);
#endif
    }
}
                         ...
                         const int *ex, const int *half)
{
    ...
    for (i=0;i<h->nsig;i++) {
        ...
        if (code[i]!=CODE_NONE) {
            ...
            log_trace(2,"rtcm3 %d: unknown signal id=%2d\n",type,h->sigs[i]);
        }
    }
    log_trace(3,"rtcm3 %d: signals=%s\n",type,msm_type);

    if (rtcm->outmsg) {
        char *msg=rtcm->msgtype+strlen(rtcm->msgtype);
        sprintf(msg," signals=%s",msm_type);
    }
    /* get signal index */
    ...
    for (i=j=0;i<h->nsat;i++) {
        ...
        if ((sat=satno(sys,prn))) {
            ...
            log_trace(2,"rtcm3 %d satellite error: prn=%d\n",type,prn);
        }
        fcn=0;
        if (sys==SYS_GLO) {
        ...
        for (k=0;k<h->nsig;k++) {
            ...
            if (sat&&index>=0&&idx[k]>=0) {
                ...
                rtcm->obs.data[index].rcv=rtcm->srcid;
            }
            j++;
        }
    }
                           ...
                           msm_h_t *h, int *hsize)
{
    ...
    *h=h0;
    ...
    if (h->nsat*h->nsig>64) {
        log_trace(2,"rtcm3 %d number of sats and sigs error: nsat=%d nsig=%d\n",
              type,h->nsat,h->nsig);
        return -1;
    }
    if (i+h->nsat*h->nsig>rtcm->len*8) {
        log_trace(2,"rtcm3 %d length error: len=%d nsat=%d nsig=%d\n",type,
    ...
    *hsize=i;
    ...
    time2str(rtcm->time,tstr,2);
    log_trace(4,"decode_head_msm: time=%s sys=%d staid=%d nsat=%d nsig=%d sync=%d iod=%d ncell=%d\n",
...
static int decode_msm4(rtcm_t *rtcm, int sys)
{
    ...
    if (i+h.nsat*18+ncell*48>rtcm->len*8) {
        log_trace(2,"rtcm3 %d length error: nsat=%d ncell=%d len=%d\n",type,h.nsat,
              ncell,rtcm->len);
        return -1;
    }
...
static int decode_type1230(rtcm_t *rtcm)
{
    ...
    if (i+20>=rtcm->len*8) {
        log_trace(2,"rtcm3 1230: length error len=%d\n",rtcm->len);
        return -1;
    }
    staid=getbitu(rtcm->buff,i,12); i+=12;
...
static int decode_type4073(rtcm_t *rtcm)
{
    ...
    log_trace(2,"rtcm3 4073: unsupported message subtype=%d\n",subtype);
    return 0;
}
...
static int decode_type4076(rtcm_t *rtcm)
{
    ...
    if (i+3+8>=rtcm->len*8) {
        log_trace(2,"rtcm3 4076: length error len=%d\n",rtcm->len);
        return -1;
    }
    ...
    log_trace(2,"rtcm3 4076: unsupported message subtype=%d\n",subtype);
    return 0;
}
...
extern int decode_rtcm3(rtcm_t *rtcm)
{
    ...
    log_trace(3,"decode_rtcm3: len=%3d type=%d\n",rtcm->len,type);
